...
func (m *Manager) StartRenterd(ctx context.Context, sk types.PrivateKey, ready chan<- struct{}) (err error) {
	...
	b, err := bus.New(ctx, ([32]byte)(sk[:32]), am, wh, cm, s, wm, store, 24*time.Hour, "https://api.siascan.com", log.Named("bus"))
	if err != nil {
	...
	w, err := worker.New(config.Worker{
		AccountsRefillInterval:   5 * time.Second,
		AllowPrivateIPs:          true,
		ContractLockTimeout:      time.Minute,
		ID:                       "worker",
